...
def process_csv_files(input_files, output_dir):
    city_data = {}  # 键为city_code，值为城市数据和降水记录
    ...
    for file_path in input_files:
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                city_code = row[4]  # 使用city_code作为唯一标识
                city_name = row[3]
                precip_values = row[6:6+len(dates)]  # 确保列数匹配
                ...
                # 处理每个降水值
                for date_str, precip in zip(dates, precip_values):
                    # 转换日期格式
                    ...
                    precip = precip.strip()
                    try:
                        precip_val = float(precip) if precip else None
                    except ValueError:
                        precip_val = None
