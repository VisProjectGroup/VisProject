...
export default {
  ...
  methods: {
    async fetchCityData() {
      try {
        const response = await fetch(`../assets/data/column/column_wind/wind_speed_data/${this.CityName}.json`);
        if (!response.ok) throw new Error();
        const module = await import(`../assets/data/column/column_wind/wind_speed_data/${this.CityName}.json`);
    ...
    drawChart() {
      ...
      const y = d3.scaleLinear()
        .domain([0, d3.max(this.filteredData, d => d.wind_speed)])  // 使用实际存在的wind speed字段
        .nice()
        .range([height, 0]);
      ...
      svg.selectAll(".bar")
        ...
        .attr("class", "bar")
        .attr("x", d => x(d.date))
        .attr("y", d => y(d.wind_speed))
        .attr("width", x.bandwidth())
        .attr("height", d => height - y(d.wind_speed))
        .attr("fill", "steelblue");
