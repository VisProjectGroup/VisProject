...
export default {
  ...
  methods: {
    async fetchCityData() {
      try {
        const response = await fetch(`../assets/data/column/column_rain/precipitation_data/${this.CityName}.json`);
        if (!response.ok) throw new Error();
        const module = await import(`../assets/data/column/column_rain/precipitation_data/${this.CityName}.json`);
    ...
    drawChart() {
      ...
      const y = d3.scaleLinear()
        .domain([0, d3.max(this.filteredData, d => d.precipitation)])  // 使用实际存在的precipitation字段
        .nice()
        .range([height, 0]);
